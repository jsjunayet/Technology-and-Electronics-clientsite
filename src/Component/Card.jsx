...
const Card = () => {
    const Myproudcts = useLoaderData()
    const [Newproduct, setNewproduct] = useState(Myproudcts)

    console.log(Myproudcts)

    const handleDelte = (_id) => {

        Swal.fire({
            title: 'Are you sure?',
            text: "You are won't be able to revert this!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                fetch(`https://backend-site-by9c5gtna-junayet-shiblus-projects.vercel.app/user/${_id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
                        if (data.deletedCount > 0) {
                            console.log(data)
                            const remaing = Newproduct.filter(product => product._id !== _id)
                            setNewproduct(remaing)
                            Swal.fire(
                                'Deleted!',
